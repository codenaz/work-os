...
export class CanonicalEventService {
  ...
  getGitHubSourceEventId(
    ...
    const issueNumber =
      payload.issue?.number ?? payload.pull_request?.number ?? 'na';
    return [
  ...
  fromGitHubEvent(
    ...
    if (!this.isSupportedGitHubEvent(payload)) {
      return null;
    }

    const sourceEventId = this.getGitHubSourceEventId(payload, deliveryId);
    const lines = [
      payload.pull_request?.title ?? payload.issue?.title ?? '',
      payload.pull_request?.body ?? payload.issue?.body ?? '',
    ...
    const text =
      lines.join('\n\n') ||
      `${payload.eventType} ${payload.action} in ${payload.repository.full_name}`;
  ...
  private isSupportedGitHubEvent(payload: GitHubWebhookDto) {
    if (payload.eventType === 'issues') {
      return ['opened', 'edited', 'reopened'].includes(payload.action);
    }
    ...
    if (payload.eventType === 'pull_request') {
      return ['opened', 'reopened', 'edited'].includes(payload.action);
    }
